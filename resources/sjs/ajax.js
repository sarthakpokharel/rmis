...
var appname="SuTRA";
var appfullname="Sub-National Treasury Regulatory Application";
var copyright="Financial Comptroller General Office";
...
function additem(combobox,text,value,className)
{
...
function datagrid(gridvar,valueelement)
	{
		...
		datagrid.prototype.setformat=function(v)
		{
			this.format=v;
			try{
				document.getElementById(this.valuecontrol+'_colname').value=v;
			}catch(e){}
		}
...
function inputvalue()
{
	...
	for (var i = 0; i < inputs.length; ++i) {
  		if(ret=="")
			...
			ret=ret + '&' +inputs[i].id + '='+inputs[i].value;
	}
	var selects=document.getElementsByTagName('select');
	for (var i = 0; i < selects.length; ++i) {
  		if(ret=="")
			ret=selects[i].id + '='+selects[i].value;
		else
			ret=ret + '&' +selects[i].id + '='+selects[i].value;
	}
	return ret;
...
function proceedserver(actionurl)
{
	...
	function encode(str) {
		...
		function decode(str) {
			...
			{
				   var a=val(document.getElementById(b).value);
    if(a=="0")
