...
function datafile(nepdate,flag)
{
	...
	y=y+flag;
	loadfile="Date"+y+".xml";
	return "<?php echo $calpath;?>db/"+loadfile;
}
       ...
       if(i <= 34)
        {
           	if(transitnepday>Counter)
				{
					engtitle=engyear + "-" + engmonth  + "-" + (engday+engcounter);
					if(engtitle==today)
				...
				{
					engcounter=1;
					engtitle=transityear + "-" + transitmonth + "-" + engcounter;
					if(engtitle==today)
						aaja=years + "/" + months + "/" + Counter;
				}
				else
...
<style>
	.calinput{
		background-image:url(<?php echo $calpath;?>normal.gif);
		background-position:right;
		background-repeat:no-repeat;
		cursor:default;
		}
	.calinputhover{
			background-image:url(<?php echo $calpath;?>click.gif);
			background-position:right;
		background-repeat:no-repeat;
		cursor:default;
			}
	.calinputclick{
		background-image:url(<?php echo $calpath;?>hover.gif);
		background-position:right;
		background-repeat:no-repeat;
		cursor:default;
		}
...
<div id="divcalendar" style="position:absolute; visibility:hidden; z-index:2501">
...
<tr>
	...
	<td colspan=4>
	<select id=month name=month style="width:100%; border:0px; list-style:outside; font-size:18px; background-color:#D5EDB3" onchange="getmonthdata(document.getElementById('year').value + '/' + this.value + '/1');">
	<option value=1 style="font-size:24px;">बैशाख</option>
	<option value=2>जेठ</option>
	<option value=3>असार</option>
	<option value=4>साउन</option>
	<option value=5>भदौ</option>
	<option value=6>अस्बिन</option>
	<option value=7>कार्तिक</option>
	<option value=8>मार्ग</option>
	<option value=9>पौष</option>
	<option value=10>माध</option>
...
<tr>
	<td>आइत</td><td>स‌ोम</td><td>मंगल</td><td>बुध</td><td>बिही</td><td>शुक्र</td><td>शनि</td>
</tr>
<tr>
...
<script language="javascript">
	...
	{
		activefield.value=activedive.title; //document.getElementById("year").value + "/" + document.getElementById("month").value + "/" + activedive.innerHTML;
		hidecalendar();
		activefield.focus();
	}
	...
	function createcalendar(myObj)
	{
		myObj.className="calinput";
		myObj.onclick=calinputmousedown;
		myObj.onmouseup=calinputmouseup;
		myObj.onmousemove=calinputmousemove;
		myObj.onmouseout=calinputmouseout;
