...
%>
var pageheight=0;
var _editor_url="<%=Rtfpath%>";
// write out styles for UI buttons
document.write('<style type="text/css">\n');
document.write('.btn     { width: 22px; height: 20px; border: 0px solid silver; margin: 0; padding: 0; background-image: url(<%=Rtfpath%>editor_includes/editor_images/footer.gif);}\n');
document.write('.btnOver { width: 22px; height: 20px; border: 1px outset; background-image: url(<%=Rtfpath%>editor_includes/editor_images/background_btn_hover.gif);}\n');
document.write('.btnDown { width: 22px; height: 20px; border: 1px inset; background-color: FFCC99; background-image: url(<%=Rtfpath%>editor_includes/editor_images/background_btn_down.gif); }\n');
document.write('.btnNA   { width: 22px; height: 20px; border: 1px solid silver; filter: alpha(opacity=25); }\n');
document.write('.cMenu     { background-color: threedface; color: menutext; cursor: Default; font-family: MS Sans Serif; font-size: 8pt; padding: 2 12 2 16;background-image:url(<%=Rtfpath%>editor_includes/editor_images/bg_s.gif) }');
...
this.bodyStyle = 'background-color: #ededed; font-family: "Verdana"; font-size: x-small;';
this.imgURL = _editor_url + 'editor_includes/editor_images/';
this.imgURL = '<%=Rtfpath%>editor_includes/editor_images/';
this.debug  = 0;
...
\* ---------------------------------------------------------------------- */
function CreateEditor(ObjName)//,ObjWidth,ObjHeight)
{
	//userWidth=ObjWidth;//userHeight=ObjHeight;
...
function editor_generate(objname,userConfig) {
  ...
  var tblOpen  = '<table border=0 cellspacing=0 cellpadding=0 style="float: left; background-image:url(<%=Rtfpath%>editor_includes/editor_images/footer.gif); height=23px;"  unselectable="on"><tr><td style="border: none; padding: 1 0 0 0"><nobr>';
  ...
  var editor = '<span id="_editor_toolbar" style="background-image:url(<%=Rtfpath%>editor_includes/editor_images/footer.gif)"><table border=0 cellspacing=0 cellpadding=0 bgcolor="silver" style="padding: 1 0 0 2" width=' + config.width + ' unselectable="on" style="background-image:url(<%=Rtfpath%>editor_includes/editor_images/footer.gif)"><tr><th>\n'
  + toolbar
  + '</th></tr></table>\n'
  + '</th></tr></table></span>\n'
  + '<textarea ID="_' +objname + '_editor" onChange="editor_action(this)" style="width:' +config.width+ '; height:' + config.height + '; margin-top: -1px; margin-bottom: -1px; overflow:visible;" wrap=soft;  ></textarea>';
...
//    var newcolor = showModalDialog(_editor_url + "editor_includes/editor_popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
    var newcolor = showModalDialog("editor_includes/editor_popups/select_color.html", oldcolor, "resizable: no; help: no; status: no; scroll: no;");
...
//      showModalDialog(_editor_url + "editor_includes/editor_popups/insert_image.html", editdoc, "resizable: yes; help: no; status: no; scroll: no; ");
      showModalDialog("editor_includes/editor_popups/insert_image.asp", editdoc, "resizable: yes; help: no; status: no; scroll: no; ");	  
    }
...
function editor_event(objname,runDelay) {
  ...
  // catch keypress events
    if (editEvent && editEvent.keyCode) {
		...
		if(editEvent.type=="keyup")
		{
		var abc;
		//abc=objname+"_div";
		//document.getElementById(abc).innerHTML=document.getElementById(objname).value;
		//pageheight=document.getElementById(abc).offsetHeight;
		...
		//	ph=pageheight%850;
			...
			//	pph=pageheight/850;
			//else
			//	pph=((pageheight-ph)/850)+1;
				
			//document.getElementById("pageno").value=pph;
			
		//}
		//document.getElementById("pageno").value=pageheight/3500;
		//document.getElementById("toprice").value=document.getElementById("pageno").value*document.getElementById("rsperpage").value;
      ...
      if (ord == 13 && editEvent.type == 'keypress') {
	   ...
	   if (ord == 13 && editEvent.type == 'keypress' && shiftKey) {
     	editEvent.returnValue = false;
        editor_insertHTML(objname, "<p>");
        // return;
       }
...
function editor_setmode(objname, mode) {
  ...
  var TextEdit   = '<textarea ID="_' +objname + '_editor" style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; margin-top: -1px; margin-bottom: -1px; overflow:visible;"></textarea>';
  var RichEdit   = '<iframe   ID="_' +objname+ '_editor"    style="width:' +editor_obj.style.width+ '; height:' +editor_obj.style.height+ '; overflow:visible;"></iframe>';
...
//  showModalDialog(_editor_url + "editor_includes/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
  showModalDialog( "editor_includes/popups/about.html", window, "resizable: yes; help: no; status: no; scroll: no; ");
}
...
config.toolbar = [
    ...
    ['OrderedList','UnOrderedList','Outdent','Indent','separator'],
    ['forecolor','backcolor','separator'],
    ['HorizontalRule','Createlink','InsertImage','htmlmode'],
