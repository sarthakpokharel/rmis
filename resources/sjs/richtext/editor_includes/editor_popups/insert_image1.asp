...
//alert(frmupload.txtFileName.value);
  // event handlers
...
<script language="javascript">
	function getfilename(r)
	{
		...
		{	var a=new String(r);
			var b=a.split("\\");
			document.getElementById('filename').value= 'editor_includes/editor_popups/upload/' + '<% set rs= server.createobject("ADODB.Recordset")
...
<%
response.write  session("messageid") &"_"& session("fileno") &"_" %>' + b[b.length-1];
%>
	}
		else
		{
...
<input type="file" id="txtFileName" name="txtFileName" style="left: 8.54em; top: 1.0647em; width: 21.5em;height: 2.1294em; " tabIndex=10  onChange="getfilename(this.value)">
<input type="text" id="filename" name="filename" value="<%

 //fn = split(o.FileNameOf("txtFileName"), "\")
// if ubound(fn)>=0 then
//response.write "editor_includes/editor_popups/upload/" & fn(Ubound(fn))
//response.write "editor_includes/editor_popups/upload/" & session("filename")
//end if
if trim(session("filename"))<>"" then
response.write "editor_includes/editor_popups/upload/" & session("filename") &"_"& session("fileno") &"_"
end if
 %>
 ">
...
function btnOKClick1() {
  ...
  // error checking
//alert(frmupload.filename.value);
...
sFileSplit = split(o.FileNameOf("txtFileName"), "\")
sFile = session("messageid") &"_"& session("fileno") &"_"& sFileSplit(Ubound(sFileSplit))
session("filename")=sFile
